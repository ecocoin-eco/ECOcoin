...
def confFile():
    ...
    if platform.system() == 'Windows':
        folder = os.path.join(os.path.join(os.environ['APPDATA'], 'ecocoin'))
    else:
        if platform.system() == 'Darwin':
            folder = os.path.expanduser('~/Library/Application Support/ecocoin/')
        else:
            folder = os.path.expanduser('~/.ecocoin')

    return os.path.join(folder, 'ecocoin.conf')

conf_path = confFile()
if not os.path.exists(conf_path):
    parser.error('''ecocoin configuration file not found. Manually enter your RPC password.\r\n'''
        ...
        '''rpcpassword=%x\r\n'''
        '''\r\n'''
        '''Keep that password secret! After creating the file, restart ecocoin.''' % (conf_path, random.randrange(2**128)))
...
for node in json.loads(data.decode()):
    ...
    try:
        ...
        if e.errno == errno.ECONNREFUSED:
            print ('Unable to communicate with ecocoin RPC')
        break
